...
class InverseKinematics(Node):
    ...
    def __init__(self):
        ...
        # Squat height modulation
        self.elapsed_time = 0.0
        self.squat_freq = 0.5  # Hz - different from gait frequency
        self.squat_amplitude = 0.02
        self.squat_base = 0.005
        ...
        squatheight = 0.005
        rf_ee_offset = np.array([0.07, -0.13, squatheight])
        # front left leg
        ...
        lf_ee_offset = np.array([0.07, 0.13, squatheight])
        # front right leg
        ...
        rb_ee_offset = np.array([-0.11, -0.13, squatheight+0.02])
        # back left leg
        ...
        lb_ee_offset = np.array([-0.11, 0.13, squatheight+0.02])
        # back right leg
        ...
        self.pd_timer_period = 1.0 / 800  # 800 Hz
    ...
    def get_target_joint_positions(self):
        ...
        target_ee = self.target_ee_cache[self.counter].copy()
        
        # Apply time-varying squat height modulation
        squat_mod = self.squat_base + self.squat_amplitude * np.sin(2 * np.pi * self.squat_freq * self.elapsed_time)
        target_ee[2] += squat_mod
    ...
    def ik_timer_callback(self):
        if self.joint_positions is not None:
            self.elapsed_time += self.ik_timer_period  # Update elapsed time for squat modulation
