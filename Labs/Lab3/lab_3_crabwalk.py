...
class InverseKinematics(Node):
    ...
    def __init__(self):
        ...
        base = 0.04
        touch_down_position = np.array([0, base, -0.14])
        stand_position_1 = np.array([0, base/2, -0.14])
        stand_position_2 = np.array([0, 0, -0.14])
        stand_position_3 = np.array([-0, -base/2, -0.14])
        liftoff_position = np.array([0, -base,  -0.14])
        mid_swing_position = np.array([0, 0,  -0.07])          
        
        rtouch_down_position = np.array([base/3.5, base, -0.14])
        rstand_position_1 = np.array([base/7, base/2, -0.14])
        rstand_position_2 = np.array([0, 0, -0.14])
        rstand_position_3 = np.array([-base/7, -base/2, -0.14])
        rliftoff_position = np.array([-base/3.5, -base,  -0.14])
        rmid_swing_position = np.array([0, 0,  -0.07]) 

        ## trotting
        # same sequence for fornt left and back rigt,
        # and front r and back left, with 0.5 phase shift

        # TODO: Implement each leg’s trajectory in the trotting gait.
        squatheight = 0.005
        rf_ee_offset = np.array([0.07, -0.09, squatheight])
        # front left leg
        rf_ee_triangle_positions = np.array([
            ################################################################################################
            # TODO: Implement the trotting gait
            ################################################################################################
            rtouch_down_position, rstand_position_1, rstand_position_2, rstand_position_3, rliftoff_position, rmid_swing_position, rtouch_down_position
        ]) + rf_ee_offset
        
        lf_ee_offset = np.array([0.07, 0.09, squatheight])
        # front right leg
        ...
        rb_ee_offset = np.array([-0.11, -0.09, squatheight+0.015])
        # back left leg
        rb_ee_triangle_positions = np.array([
            ################################################################################################
            # TODO: Implement the trotting gait
            ################################################################################################
            rstand_position_3, rliftoff_position, rmid_swing_position, rtouch_down_position, rstand_position_1, rstand_position_2, rstand_position_3
        ]) + rb_ee_offset
        
        lb_ee_offset = np.array([-0.11, 0.09, squatheight+0.015])
        # back right leg
        lb_ee_triangle_positions = np.array([
            ################################################################################################
            # TODO: Implement the trotting gait
            ################################################################################################
            touch_down_position, stand_position_1, stand_position_2, stand_position_3, liftoff_position, mid_swing_position, touch_down_position
        ]) + lb_ee_offset
    ...
    def get_error_leg(self, theta, desired_position):
        ...
        err = np.mean((desired_position - self.leg_forward_kinematics(theta)) ** 2)
    ...
    def inverse_kinematics_single_leg(self, target_ee, leg_index, initial_guess=[0, 0, 0]):
        ...
        result = scipy.optimize.minimize(self.get_error_leg, 
    ...
    def interpolate_triangle(self, t, leg_index):
        ...
        num_positions = triangle_positions.shape[0] - 1.0

        delta = 1.0 / (num_positions)
        # print(num_positions, delta)

        if t <= 1*delta:
            return triangle_positions[0] + (triangle_positions[1] - triangle_positions[0]) * (t / delta)
        elif t <= 2*delta:
            return triangle_positions[1] + (triangle_positions[2] - triangle_positions[1]) * ((t - 1*delta) / delta)
        elif t <= 3*delta:
            return triangle_positions[2] + (triangle_positions[3] - triangle_positions[2]) * ((t - 2*delta) / delta)
        elif t <= 4*delta:
            return triangle_positions[3] + (triangle_positions[4] - triangle_positions[3]) * ((t - 3*delta) / delta)
        elif t <= 5*delta:
            return triangle_positions[4] + (triangle_positions[5] - triangle_positions[4]) * ((t - 4*delta) / delta)
        else:
            return triangle_positions[5] + (triangle_positions[6] - triangle_positions[5]) * ((t - 5*delta) / delta) 
        ################################################################################################
